...
if __name__ == "__main__":
    ...
    # weight from MC
    weight = mc_arrays["weight"].astype(np.float64)   # changed genWeight → weight
    ...
    print(f"  w            = {w:.6e}")
    print(f"  weight mean  = {weight.mean():.2f}")
    print(f"  weight sum   = {weight.sum():.4e}")
    print(f"  MC N events  = {len(mc_arrays['Probe_pt'])}")
    print(f"  Data N events= {len(data_arrays['Probe_pt'])}")
    print(f"  MC expected yield = {weight.sum() * w:.2f}")
    print(f"  Data yield        = {len(data_arrays['Probe_pt'])}")
